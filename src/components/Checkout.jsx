...
export default function Checkout() {
  ...
  useEffect(() => {
    async function loadCart() {
      try {
        // If state is an array (Buy Now), use that; otherwise, fetch from backend
          ...
          items.map(item =>
            fetchProductById(item.productId)
              ...
              .catch(err => {
                ...
                // You could set details[item.productId] = null here if you want to show an error placeholder
              })
          )
        );
        setProducts(details);
  ...
  const handleSubmit = async e => {
    ...
    const subtotal = cartItems.reduce((sum, item) => {
      ...
      if (!p) return sum; // skip if product details not loaded / missing
  ...
  const subtotal = cartItems.reduce((sum, item) => {
    const p = products[item.productId];
    if (!p) return sum; // skip if product missing
    const unit = p.discount
      ? p.price * (1 - p.discount / 100)
      : p.price;
    return sum + unit * item.qty;
    ...
    <div className="checkout-container">
      ...
      <div className="checkout-grid">
        ...
        <aside className="order-summary">
          ...
          {cartItems.map(item => {
            const p = products[item.productId];
            if (!p) return null; // skip if product wasn’t fetched
