...
export default function Tshirt() {
  ...
  // 2) Load “tshirt” category ID and subcategories on mount
  useEffect(() => {
    (async () => {
      try {
        ...
        // Adjust this if your backend’s category name is “tshirts” (plural) instead of “tshirt”
        const tshirtCat = cats.find(c => c.name.toLowerCase() === 't-shirt');
        if (!tshirtCat) {
          console.error('[DEBUG] Categories fetched but no “tshirt” found');
          throw new Error('tshirt category not found');
        }

        console.log('[DEBUG] Found tshirts category:', tshirtCat);
        setCategoryId(tshirtCat._id);
        setSubs(tshirtCat.subcategories || []);
        console.log('[DEBUG] Set subs to:', tshirtCat.subcategories);
      } catch (e) {
  ...
  const loadItems = async (subId) => {
    ...
    try {
      ...
      console.error('[DEBUG] Error loading tshirts:', e);
      setError('Failed to load tshirt');
    } finally {
      setLoading(false);
    }
  };
