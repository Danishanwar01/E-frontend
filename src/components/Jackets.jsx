...
export default function Jackets() {
  ...
  // 2) Load “jacket” category ID and subcategories on mount
  useEffect(() => {
    (async () => {
      try {
        ...
        // Adjust this if your backend’s category name is “jackets” (plural) instead of “jacket”
        const jacketsCat = cats.find(c => c.name.toLowerCase() === 'jackets');
        if (!jacketsCat) {
          console.error('[DEBUG] Categories fetched but no “jackets” found');
          throw new Error('jackets category not found');
        }

        console.log('[DEBUG] Found jackets category:', jacketsCat);
        setCategoryId(jacketsCat._id);
        setSubs(jacketsCat.subcategories || []);
        console.log('[DEBUG] Set subs to:', jacketsCat.subcategories);
      } catch (e) {
  ...
  const loadItems = async (subId) => {
    ...
    try {
      ...
      console.error('[DEBUG] Error loading jackets:', e);
