...
export default function Trousers() {
  ...
  // 2) Load “trousers” category ID and subcategories on mount
  useEffect(() => {
    (async () => {
      try {
        ...
        // Adjust this if your backend’s category name is “trouserss” (plural) instead of “trousers”
        const trousersCat = cats.find(c => c.name.toLowerCase() === 'trousers');
        if (!trousersCat) {
          console.error('[DEBUG] Categories fetched but no “trousers” found');
          throw new Error('trousers category not found');
        }

        console.log('[DEBUG] Found trouserss category:', trousersCat);
        setCategoryId(trousersCat._id);
        setSubs(trousersCat.subcategories || []);
        console.log('[DEBUG] Set subs to:', trousersCat.subcategories);
      } catch (e) {
  ...
  const loadItems = async (subId) => {
    ...
    try {
      ...
      console.error('[DEBUG] Error loading trouserss:', e);
